...
// 先建立一个索引,问题是元音字母的场景处理，我们可以把元音字母统一标记一个特殊的字符
func spellchecker(wordlist []string, queries []string) []string {
	...
	for i := range queries {
		query := queries[i]
		if id, ok := index1[query]; ok {
			res[i] = wordlist[id]
		} else if id, ok := index2[strings.ToLower(query)]; ok {
			res[i] = wordlist[id]
		} else if id, ok := index3[convert(strings.ToLower(query))]; ok {
			res[i] = wordlist[id]
		}
	}
	return res
