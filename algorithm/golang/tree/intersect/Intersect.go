...
func doBuildGrid(grid [][]int, node *Node, p1 []int, p2 []int) {
	if node == nil {
		return
	}
	if node.IsLeaf {
		...
		xMid := (p1[0] + p2[0]) / 2
		yMid := (p1[1] + p2[1]) / 2
		doBuildGrid(grid, node.TopLeft, p1, []int{xMid, yMid})
		doBuildGrid(grid, node.TopRight, []int{p1[0], yMid + 1}, []int{xMid, p2[1]})
		doBuildGrid(grid, node.BottomLeft, []int{xMid + 1, p1[1]}, []int{p2[0], yMid})
		doBuildGrid(grid, node.BottomRight, []int{xMid + 1, yMid + 1}, []int{p2[0], p2[1]})
	}
}

func findDepth(node *Node) int {
	if node == nil {
		return 0
	}
	if node.IsLeaf {
	...
	mx := 0
	depth := findDepth(node.TopLeft)
	if depth > mx {
		mx = depth
	}
	depth = findDepth(node.TopRight)
	if depth > mx {
		mx = depth
	}
	depth = findDepth(node.BottomLeft)
	if depth > mx {
		mx = depth
	}
	depth = findDepth(node.BottomRight)
	if depth > mx {
		mx = depth
	}
	return mx + 1
