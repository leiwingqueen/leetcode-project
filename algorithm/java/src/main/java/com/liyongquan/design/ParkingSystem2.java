...
public class ParkingSystem2 {
    ...
    public ParkingSystem2(int big, int medium, int small) {
        cnt = new AtomicInteger[]{new AtomicInteger(big), new AtomicInteger(medium), new AtomicInteger(small)};
    }

    public boolean addCar(int carType) {
        int num = cnt[carType - 1].get();
        if (num <= 0) {
            return false;
        }
        while (num > 0) {
            if (cnt[carType - 1].compareAndSet(num, num - 1)) {
                return true;
            }
            num = cnt[carType - 1].get();
        }
        return false;
    }
}
